...
from experiments.plotting.plot_ma_sam_results import plot_results
...
class StatisticsCollector:
    ...
    def _process_combined_data(combined_statistics_data: List[Dict]) -> pd.DataFrame:
        ...
        ma_sam_columns = ["percent_ok", "fold", "num_trajectories"]
        df[ma_sam_columns] = df[ma_sam_columns].apply(pd.to_numeric)
        # df = df[~(df['num_trajectories'] > num_traj)]
        return df
    ...
    def _collect_solving_statistics(self) -> None:
        ...
        with open(combined_statistics_file_path, "wt") as combined_statistics_file:
            writer = csv.DictWriter(combined_statistics_file, fieldnames=[FOLD_FIELD, POLICY, *SOLVING_STATISTICS])
            writer.writeheader()
    ...
    def collect_statistics(self) -> None:
        ...
        #TODO adapt to masam performance statistics
