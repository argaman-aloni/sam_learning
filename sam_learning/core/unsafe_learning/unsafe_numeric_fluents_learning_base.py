...
class UnsafeFluentsLearning(ABC):
    ...
    def __init__(
        self, action_name: str, polynomial_degree: int = 0, partial_domain: Union[Domain, LearnerDomain] = None
    ):
    ...
    def _add_polynomial(self, lifted_fluents: Dict[str, PDDLFunction], dataset: Dict[str, List[float]]) -> None:
        ...
        for degree in range(2, self.polynom_degree + 1):
            for fluent_combination in itertools.combinations_with_replacement(list(lifted_fluents.keys()), r=degree):
    ...
    def _add_lifted_post_state_fluent_to_dataset(
        self, component: ObservedComponent, dataset: Dict[str, List[float]]
    ) -> None:
        ...
        """
        matches = self.function_matcher.match_state_functions(
            component.grounded_action_call, component.next_state.state_fluents
        )
        for fluent_name, func in matches.items():
            dataset[fluent_name].append(func.value)

    def _add_lifted_functions_to_dataset(self, component: ObservedComponent, dataset: Dict[str, List[float]]) -> None:
        ...
        """
        matches = self.function_matcher.match_state_functions(
            component.grounded_action_call, component.previous_state.state_fluents
        )
        for fluent_name, func in matches.items():
            dataset[fluent_name].append(func.value)
    ...
    def _filter_out_inconsistent_state_variables(dataset: Dict[str, List[float]]) -> Dict[str, List[float]]:
        ...
        return {
            lifted_function: state_values
            for lifted_function, state_values in dataset.items()
            if len(state_values) == max_function_len
        }

    def _create_pre_state_classification_dataset(
    ...
    def _create_inequality_constraint_strings(
        self,
        features_names: List[str],
        coefficients_path: List[List[float]],
        intercepts_path: List[float],
        should_be_also_equal: bool = True,
    ) -> List[str]:
    ...
    def _construct_regression_data(
        self, positive_observations: List[Observation]
    ...
    def learn_effects(self, positive_observations: List[Observation]) -> List[str]:
        ...
        for fluent_name in post_state_values:
            ...
            if all(
                post_value - pre_value == 0
                for pre_value, post_value in zip(pre_state_values[fluent_name], post_state_values[fluent_name])
            ):
            ...
            if fluent_name in coefficients_map:
                assignment_statements.append(
                    construct_non_circular_assignment(
                        fluent_name,
                        coefficients_map,
                        pre_state_values[fluent_name][0],
