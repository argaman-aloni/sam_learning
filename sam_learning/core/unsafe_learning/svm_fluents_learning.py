...
from sam_learning.core.numeric_learning import prettify_coefficients, prettify_floating_point_number
...
class SVMFluentsLearning(UnsafeFluentsLearning):
    ...
    def _remove_rows_with_accurate_classification(
        self, input_df: pd.DataFrame, coefficients: List[float], intercept: float
    ) -> pd.DataFrame:
    ...
    def learn_preconditions(
        ...
        current_iteration = 0
        while (
            current_iteration < MAX_ALLOWED_ITERATIONS
        ...
        return (
            self._create_inequality_constraint_strings(
                feature_names, coefficients_route, intercept_route, should_be_also_equal=False
            ),
