...
class FaultGenerator:
    ...
    def alter_action_numeric_precondition(self, faulty_action: Action) -> None:
    ...
    @staticmethod
    def _alter_numeric_expression(expression: NumericalExpressionTree, should_decrease: bool = False) -> None:
        ...
        if isinstance(node.value, float):
            ...
            new_add_node = AnyNode(
                ...
                children=[AnyNode(id=function_name, value=function_value), AnyNode(id=str(alter_by), value=alter_by)],
            )
            left_sibling = node.parent.children[0]
            node.parent.children = (left_sibling, new_add_node)

    def alter_action_numeric_precondition_value(self, faulty_action: Action) -> None:
    ...
    def alter_action_numeric_effect(self, faulty_action: Action) -> None:
        """Alter the action's effects so that it will contain a defect.
    ...
    def remove_predicate_from_action(self, faulty_action: Action) -> None:
    ...
    def remove_numeric_precondition(self, faulty_action: Action) -> None:
    ...
    def _alter_action_according_to_defect_type(self, faulty_action: Action, defect_type: DefectType) -> None:
    ...
    def _alter_action_according_to_random_defect(self, faulty_action: Action) -> None:
    ...
    def _set_faulty_domain_and_defected_action(
        self, defect_type: Optional[DefectType] = None, action_to_alter: Optional[str] = None
    ) -> Domain:
        ...
        faulty_action = self._select_action_to_alter(altered_domain) if action_to_alter is None else altered_domain.actions[action_to_alter]
    ...
    def generate_faulty_domain(self, defect_type: Optional[DefectType] = None, action_to_alter: Optional[str] = None) -> Domain:
