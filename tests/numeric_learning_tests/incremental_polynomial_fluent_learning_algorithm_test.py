...
from sam_learning.core.numeric_learning import IncrementalPolynomialFluentsLearningAlgorithm
...
def test_create_monomials_creates_correct_monomials_when_given_polynom_degree_one():
    polynomial_fluent_storage = IncrementalPolynomialFluentsLearningAlgorithm(
    ...
    polynomial_fluent_storage._create_monomials(test_functions)
    monomials = polynomial_fluent_storage.monomials
...
def test_create_monomials_creates_correct_monomials_when_given_polynom_degree_zero():
    polynomial_fluent_storage = IncrementalPolynomialFluentsLearningAlgorithm(
        action_name="load", domain_functions=TEST_DOMAIN_FUNCTIONS, polynom_degree=0
    )
    test_functions = ["(fuel-cost )", "(load_limit ?z)", "(current_load ?z)"]
    polynomial_fluent_storage._create_monomials(test_functions)
    monomials = polynomial_fluent_storage.monomials
...
def test_init_numeric_datasets_creates_convex_hull_dataframe_with_all_permutation_of_function_names_of_up_to_size_two_when_polynom_degree_one():
    polynomial_fluent_storage = IncrementalPolynomialFluentsLearningAlgorithm(
        action_name="load", domain_functions=TEST_DOMAIN_FUNCTIONS, polynom_degree=1
    )
    polynomial_fluent_storage.init_numeric_datasets()
    assert len(polynomial_fluent_storage.convex_hull_learner.data.columns.tolist()) == 6
    assert set(polynomial_fluent_storage.convex_hull_learner.data.columns.tolist()) == {
        "(fuel-cost )",
        "(load_limit ?z)",
        "(current_load ?z)",
...
def test_add_to_previous_state_storage_when_polynom_degree_one_creates_con():
    polynomial_fluent_storage = IncrementalPolynomialFluentsLearningAlgorithm(
...
def test_add_to_previous_state_storage_adds_correct_keys_combinations_with_polynom_degree_one():
    polynomial_fluent_storage = PolynomialFluentsLearningAlgorithm(action_name="load", polynom_degree=1, domain_functions=TEST_DOMAIN_FUNCTIONS)
    FUEL_COST_FUNCTION.set_value(2.0)
    LOAD_LIMIT_TRAJECTORY_FUNCTION.set_value(3.0)
    simple_state_fluents = {
        "(fuel-cost )": FUEL_COST_FUNCTION,
...
def test_add_to_previous_state_storage_adds_correct_values_combinations_with_polynom_degree_two():
    polynomial_fluent_storage = PolynomialFluentsLearningAlgorithm(action_name="load", polynom_degree=2, domain_functions=TEST_DOMAIN_FUNCTIONS)
    FUEL_COST_FUNCTION.set_value(2.0)
    LOAD_LIMIT_TRAJECTORY_FUNCTION.set_value(3.0)
    CURRENT_LOAD_TRAJECTORY_FUNCTION.set_value(4.0)
    simple_state_fluents = {
...
def test_add_to_previous_state_storage_adds_correct_values_combinations_with_polynom_degree_three():
    polynomial_fluent_storage = PolynomialFluentsLearningAlgorithm(action_name="load", polynom_degree=3, domain_functions=TEST_DOMAIN_FUNCTIONS)
    FUEL_COST_FUNCTION.set_value(2.0)
    LOAD_LIMIT_TRAJECTORY_FUNCTION.set_value(3.0)
    CURRENT_LOAD_TRAJECTORY_FUNCTION.set_value(4.0)
    simple_state_fluents = {
