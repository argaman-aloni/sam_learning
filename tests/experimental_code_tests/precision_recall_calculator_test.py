...
from pddl_plus_parser.models import Predicate, Domain, Action
...
def test_calculate_recall_with_extra_learned_precondition_results_in_lower_recall():
    ...
    learned_preconditions = {p.untyped_representation for p in TEST_NOT_SYMMETRIC_LEARNED_PRECONDITIONS}
    expected_preconditions = {p.untyped_representation for p in TEST_NOT_SYMMETRIC_EXPECTED_PRECONDITIONS}
    assert calculate_syntactic_recall(learned_preconditions, expected_preconditions) == expected_recall


def test_calculate_precision_does_not_divide_by_zero():
    try:
        calculate_syntactic_precision(set([]), {p.untyped_representation for p in TEST_NOT_SYMMETRIC_EXPECTED_PRECONDITIONS})
...
def test_calculate_recall_does_not_divide_by_zero():
    try:
        calculate_syntactic_recall(set([]), {p.untyped_representation for p in TEST_NOT_SYMMETRIC_EXPECTED_PRECONDITIONS})
...
def test_add_action_data_with_learned_action_stores_correct_values_in_object(
    ...
    learned_action = Action(name=TEST_ACTION_NAME, signature=expected_action.signature)
    ...
    learned_preconditions = {p.untyped_representation for p in TEST_SYMMETRIC_LEARNED_PRECONDITIONS}
    expected_preconditions = {p.untyped_representation for p in expected_action.preconditions.root.operands}
    ...
    assert len(precision_recall_calculator.discrete_effects[TEST_ACTION_NAME]) == 0
