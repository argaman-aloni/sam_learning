...
def test_apply_unit_propagation_when_unit_clause_exists_that_is_part_on_a_non_unit_clause_cnf_removes_no_unit_clause_from_must_be_preconditions(
    ...
    non_unit_clause = {p for p in lifted_vocabulary if p.untyped_representation in ["(treatment ?x ?oldtreatment)", "(available ?x)"]}
...
def test_add_negative_pre_state_observation_adds_correct_set_of_missing_preconditions_to_the_must_be_preconditions_set(
    ...
    new_negative_sample = {
        p for p in lifted_vocabulary if p.untyped_representation in ["(not (available ?x))", "(not (treatment ?x ?oldtreatment))"]
    }
...
def test_add_positive_post_state_observation_when_adding_sample_for_the_second_time_adds_additional_predicates_to_the_cannot_be_effects_set(
    ...
    pre_state_predicates = {
        p
        for p in depot_lifted_vocabulary
        if p.untyped_representation
        not in ["(not (at ?x ?p))", "(not (available ?x))", "(not (at ?y ?p))", "(not (clear ?y))", "(not (at ?z ?p))"]
    }
    next_state_predicates = {
        p
        for p in depot_lifted_vocabulary
        if p.untyped_representation
        not in ["(at ?y ?p)", "(clear ?y)", "(not (lifting ?x ?y))", "(available ?x)", "(not (clear ?z))", "(not (at ?z ?p))"]
    }
    ...
    pre_state_predicates = {
        ...
        for p in depot_lifted_vocabulary
        if p.untyped_representation
        not in ["(not (at ?x ?p))", "(not (available ?x))", "(not (at ?y ?p))", "(not (clear ?y))", "(at ?z ?p)"]
    }
    next_state_predicates = {
        p
        for p in depot_lifted_vocabulary
        if p.untyped_representation
        not in ["(at ?y ?p)", "(clear ?y)", "(not (lifting ?x ?y))", "(available ?x)", "(not (clear ?z))", "(at ?z ?p)"]
    }
    depot_online_discrete_model_learner._add_positive_post_state_observation(
        pre_state_predicates=pre_state_predicates, post_state_predicates=next_state_predicates
    )
...
def test_add_positive_post_state_observation_when_adding_observation_when_some_predicates_declared_to_not_be_effects_remove_from_must_be_effects(
    depot_online_discrete_model_learner: OnlineDiscreteModelLearner, depot_lifted_vocabulary: Set[Predicate]
):
    pre_state_predicates = {
        p
    ...
    assert depot_online_discrete_model_learner.must_be_effects.intersection(depot_online_discrete_model_learner.cannot_be_effects) == set()
