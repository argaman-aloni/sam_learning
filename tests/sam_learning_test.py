...
def test_update_action_preconditions_reduces_the_number_of_positive_predicates_but_does_not_remove_actual_preconditions(
    ...
    observed_objects = elevators_observation.grounded_objects
    sam_learning.current_trajectory_objects = observed_objects
    sam_learning._create_fully_observable_triplet_predicates(current_action=test_action_call,
    ...
    sam_learning._create_fully_observable_triplet_predicates(
        current_action=second_test_action_call, previous_state=second_previous_state, next_state=second_next_state)
    sam_learning._update_action_preconditions(grounded_action=second_test_action_call)
    positive_preconditions = sam_learning.partial_domain.actions["move-down-slow"].positive_preconditions
...
def test_update_action_preconditions_reduces_the_number_of_negative_predicates_but_does_not_remove_actual_preconditions(
    ...
    sam_learning._create_fully_observable_triplet_predicates(
        current_action=second_test_action_call, previous_state=second_previous_state, next_state=second_next_state)
    sam_learning._update_action_preconditions(grounded_action=second_test_action_call)
    positive_preconditions = sam_learning.partial_domain.actions["move-down-slow"].negative_preconditions
    preconditions_str = set([p.untyped_representation for p in positive_preconditions])
    assert len(preconditions_str) > 0
    print(sam_learning.partial_domain.actions["move-down-slow"].to_pddl())
...
def test_add_new_action_maintains_the_correct_add_and_delete_effects(
    ...
    observed_objects = elevators_observation.grounded_objects
    sam_learning.current_trajectory_objects = observed_objects
    sam_learning.add_new_action(test_action_call, previous_state, next_state)
    learned_action = sam_learning.partial_domain.actions[test_action_call.name]
...
def test_update_action_does_not_remove_effects_from_the_action(
    ...
    sam_learning._create_fully_observable_triplet_predicates(
        current_action=second_test_action_call, previous_state=second_previous_state, next_state=second_next_state)
    sam_learning.update_action(grounded_action=second_test_action_call, previous_state=second_previous_state,
                               next_state=second_next_state)
    learned_action = sam_learning.partial_domain.actions[second_test_action_call.name]
    add_effects = {eff.untyped_representation for eff in learned_action.add_effects}
    delete_effects = {eff.untyped_representation for eff in learned_action.delete_effects}
    assert add_effects == {"(lift-at ?lift ?f2)"}
    assert delete_effects == {"(not (lift-at ?lift ?f1))"}
...
def test_handle_action_effects_does_not_create_intersecting_sets_of_effects(sam_learning: SAMLearner,
...
def test_handle_single_trajectory_component_maintains_the_correct_preconditions_invariants_after_multiple_executions(
    ...
    learned_action_data = sam_learning.partial_domain.actions[observation_component.grounded_action_call.name]
    preconditions_str = set([p.untyped_representation for p in learned_action_data.positive_preconditions])
    assert preconditions_str.issuperset(["(lift-at ?lift ?f1)", "(above ?f2 ?f1)", "(reachable-floor ?lift ?f2)"])
...
def test_learn_action_model_does_not_obstruct_the_correctness_of_the_actions(
        sam_learning: SAMLearner, elevators_observation: Observation):
    observation_component = elevators_observation.components[0]
    learned_model, _ = sam_learning.learn_action_model([elevators_observation])
