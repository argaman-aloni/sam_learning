...
from tests.consts import FUEL_COST_FUNCTION, LOAD_LIMIT_TRAJECTORY_FUNCTION, TRUCK_TYPE
...
def test_create_polynomial_string_returns_correct_string(oblique_tree_fluents_learning_first_degree_polynom: ObliqueTreeFluentsLearning):
...
def test_add_polynomial_adds_correct_polynom_when_polynomial_degree_is_one(
    ...
    oblique_tree_fluents_learning_first_degree_polynom._add_polynomial(lifted_fluents=simple_state_fluents, dataset=test_dataset)
...
def test_add_polynomial_adds_correct_polynom_when_polynomial_degree_is_two(depot_domain: Domain):
    ...
    storage_keys = list(test_dataset.keys())
    assert storage_keys == ["(* (fuel-cost ) (fuel-cost ))", "(* (fuel-cost ) (load_limit ?z))", "(* (load_limit ?z) (load_limit ?z))"]
...
def test_create_pre_state_classification_dataset_with_no_negative_observations_creates_correct_dataset(
    ...
    drive_action_components = [component for component in depot_observation.components if component.grounded_action_call.name == "drive"]
...
def test_construct_linear_equation_string_returns_correct_string(oblique_tree_fluents_learning_zero_degree_polynom: ObliqueTreeFluentsLearning):
    multiplication_parts = ["(* (fuel-cost ) (load_limit ?z))", "(* (load_limit ?z) (current_load ?z))"]
    linear_equation_string = oblique_tree_fluents_learning_zero_degree_polynom._construct_linear_equation_string(multiplication_parts)
    assert linear_equation_string == "(+ (* (fuel-cost ) (load_limit ?z)) (* (load_limit ?z) (current_load ?z)))"


def test_create_inequality_constraint_strings_returns_correct_strings(oblique_tree_fluents_learning_zero_degree_polynom: ObliqueTreeFluentsLearning):
    feature_names = ["(fuel-cost )", "(load_limit ?x)", "(current_load ?x)"]
    coefficients_paths = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
    intercept_path = [10, 11, 12]
    inequality_constraint_strings = oblique_tree_fluents_learning_zero_degree_polynom._create_inequality_constraint_strings(
...
def test_learn_preconditions_with_single_negative_observations_learns_preconditions(
    ...
    result = oblique_tree_fluents_learning_zero_degree_polynom.learn_preconditions(positive_observations, negative_observations)
    print(result)
